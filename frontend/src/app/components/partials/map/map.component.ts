...
@Component({
  selector: 'map',
  standalone: true,
  imports: [CommonModule, RouterModule, CheckoutPageComponent, OrderItemsListComponent, CartPageComponent, HomeComponent],
...
export class MapComponent implements OnInit {
  ...
  setMarker(latlng:LatLngExpression){
    this.addresLatLng = latlng as LatLng;
    ...
    this.currentMarker = marker(latlng, {
      draggable: true, icon: this.MARKER_ICON
    }).addTo(this.map);

    this.currentMarker.on('dragend', () =>{
      this.addresLatLng = this.currentMarker.getLatLng();
    })
  }
